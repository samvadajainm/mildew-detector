...
draft3_yoloSeg subfolder. Render's start command: gunicorn app:app"""
import sys
import os

# Add the draft3 folder to Python's module search path so plantseg imports work
sys.path.insert(0, os.path.join(os.path.dirname(__file__), "draft3_yoloSeg"))
...
plantseg.WEIGHTS = os.path.join(os.path.dirname(__file__),
                                "draft3_yoloSeg", "model", "plantseg_v2.pt")
